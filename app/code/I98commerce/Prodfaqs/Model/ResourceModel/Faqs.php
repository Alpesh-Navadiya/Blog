...
class Faqs extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('i98commerce_faq', 'faq_id');
    }
    ...
    public function getIsUniqueIdentifier(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $select = $this->getConnection()->select()->from(['f' => $this->getMainTable()])->where('f.identifier = ?', $object->getData('identifier'));
    ...
    public function getProducts(\I98commerce\Prodfaqs\Model\Faqs $object)
    {
        ...
        $select = $this->getConnection()->select()->from(
            $this->getTable('i98commerce_faqs_product'),
            ['product_id']
        )->where(
            'faq_id = ?',
            (int)$object->getId()
    ...
    public function getAnswers(\I98commerce\Prodfaqs\Model\Faqs $object)
    {

        $select = $this->getConnection()->select()->from(
            $this->getTable('i98commerce_faq_answers'),
            ['answer_id']
        )->where(
            'faq_id = ?',
            (int)$object->getId()
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if (isset($products)) {
            ...
            $this->getConnection()->delete($this->getTable('i98commerce_faqs_product'), $condition);
            foreach ($productIds as $_product) {
                $newsArray = [];
                $newsArray['faq_id'] = $object->getId();
                $newsArray['product_id'] = $_product;
                $this->getConnection()->insert($this->getTable('i98commerce_faqs_product'), $newsArray);
            }
        }
        ...
        if (isset($links)) {
            ...
            $condition = $this->getConnection()->quoteInto('faq_id = ?', $object->getId());
            // echo $condition;exit;
            $this->getConnection()->delete($this->getTable('i98commerce_faq_answers'), $condition);
            ...
            foreach ($productIds as $_product) {
                ...
                if (isset($_product['answer_detail']) && $_product['answer_detail']=='admin') {
                ...
                //   print_r($newsArray);
                $this->getConnection()->insert($this->getTable('i98commerce_faq_answers'), $newsArray);
            }
            // exit;
        }
